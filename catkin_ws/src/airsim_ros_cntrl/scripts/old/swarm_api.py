...
class Swarm:
    def __init__(self, swarmName, settingsFilePath=None):
        ...
        for i in self.vehicle_list:
            self.drones[i] = drone_api.Drone(self.swarm_name, i, self.client, self.lock)
...
if __name__ == "__main__":
    ...
    swarm.cmd_vel(cmd_all=drone_api.makeVelCmd(frame="world", dur=5, lz=-2))
    ...
    swarm.cmd_vel(cmd_all=drone_api.makeVelCmd(frame="body", dur=time_wait, lx=lin_vel, az=angular_vel), wait=True)
    ...
    swarm.cmd_pos(cmd_all=drone_api.makePosCmd(frame="body", y=-5, vel=2, timeout=10), wait=True)
    ...
    print("DESCENDING")
    swarm.cmd_vel(cmd_all=drone_api.makeVelCmd(frame="body", lz=2, dur=5), wait=False)
    time.sleep(5)
