...
class Target(Node):
    ...
    def __setup_ros(self) -> None:
        ...
        rclpy.Service(shutdown_service_name, SetBool, self.__handle_shutdown)
        self.multirotor_pub = rclpy.Publisher(state_topic, Multirotor, queue_size=10)

    def __handle_shutdown(self, req: SetBool.Request) -> SetBool.Response:
        """
        ...
        """
        self._shutdown = True
        return SetBool.Response(True, "")

    def shutdown(self) -> None:
        """
        Handle improper rclpy shutdown. Uses Python API to disarm drone
    ...
    def publish_multirotor_state(
        ...
        msg.looptime = Float32(rclpy.get_clock().now() - self.prev_loop_time)
        self.prev_loop_time = rclpy.get_clock().now()
    ...
    def generate_linear_velocity(
        self, current_velocity: float, position: airsim.Vector3r, waypoint: airsim.Vector3r
    ):
    ...
    def run(self) -> None:
        ...
        traj_init_time = rclpy.get_clock().now()
        ...
        while not rclpy.is_shutdown() and self._shutdown == False and time - traj_init_time < traj_time:
            ...
            angular_acc_v = airsim.Vector3r(0, 0, angular_acc)
            v_yaw_v = airsim.Vector3r(0, 0, v_yaw)
            ...
            self.time_step = rclpy.get_clock().now() - time
            time = rclpy.get_clock().now()
